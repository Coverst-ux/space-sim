...
def step_simulation():
    global paused, steps_taken, last_error
    if paused:
        return
    ...
    if not state_is_finite(physics_bodies):
        paused = True
        last_error = "Physics state became non-finite"
        return
...
while running:
    ...
    fps = clock.get_fps()
    status_text = f"FPS: {fps:.0f} | N: {len(render_bodies)} | t={elapsed_time:.0f}s"
    if paused and last_error:
        status_text = f"{status_text} | paused: {last_error}"
    fps_text = font.render(status_text, True, (180, 180, 180))
